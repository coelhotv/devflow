...
*Version-control `.agent/memory/` (indexes, detail files, journal, `process-friction.jsonl`, `attempts.jsonl`) and `.agent/constitution.md`. `.agent/state.json`, `.agent/sessions/` and `.agent/evolution/` are local and git-ignored by `scripts/setup.sh`.*
