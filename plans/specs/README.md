...
| 003 | [rc6-engine-resilience](003-rc6-engine-resilience/) | 1 | delivered (código, sem PR) | RC6: `coverage.partial` mentia com falha do motor (dosiq#835, 1/4 chunks reportado como cheio). Cobertura por passe (contrato aditivo) + retry/backoff/fallback `gemini-3.7-flash-medium`/breaker no `@core` + status file e VERDICT p/ o agente. Branch `spec/003-rc6-engine-resilience`. |
