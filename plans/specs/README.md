...
| 002 | [setup-active-ledger](002-setup-active-ledger/) | 1 | delivered | `setup.sh` idempotente (rerun APAGAVA a memória), cria ledgers com marco de início e `.gitignore`. Marco de início do ledger: derivado do git (Q1). PO-1..3 fechadas. PR #2 (merged 2026-09-22). |
| 003 | [rc6-engine-resilience](003-rc6-engine-resilience/) | 1 | in review | RC6: `coverage.partial` mentia com falha do motor (dosiq#835, 1/4 chunks reportado como cheio). Cobertura por passe (contrato aditivo) + retry/backoff/fallback `gemini-3.7-flash-medium`/breaker no `@core` + status file e VERDICT p/ o agente. Branch `spec/003-rc6-engine-resilience`, PR #3. |
